...
public class GestureComputing {
          ...
          // recognizing method
	public static GestureTemplate recognizingFile(String username, List<GestureTemplate> randomTemplates,GestureTemplate c) {
		double best = Double.MAX_VALUE;
		int templateIndex = Integer.MIN_VALUE;
		int i=0;
		int n = randomTemplates.size();
		GestureTemplate minTemplate = null;
                  //processing the points
		while(i < n) {
			double angleDistance = 0.0;
			GestureTemplate random = randomTemplates.get(i);
			//ArrayList<Point> points = random.points;
                        //ArrayList<Point> points = randomTemplates.get(i).points;//added
			angleDistance = helper(c.points, -1.0*CanvasBoard.theta, CanvasBoard.theta, CanvasBoard.dtheta, random);
		...
		minTemplate = templateIndex == Integer.MIN_VALUE ? c : randomTemplates.get(templateIndex);
		return minTemplate;
		
	}
	
	public static GestureScore scoreCalculator(String username, List<GestureTemplate> templates, ArrayList<GesturePoint> points) {
		double best = Double.MAX_VALUE;
		int templateIndex = Integer.MIN_VALUE;
		int i=0;
		...
		while(i < n) {
			double angleDistance = 0.0;
			angleDistance = helper(points, -1.0*CanvasBoard.theta, CanvasBoard.theta, CanvasBoard.dtheta, templates.get(i));
			if(angleDistance < best) {
				best = angleDistance;
				templateIndex = i;
			}
		...
		return new GestureScore(templates.get(templateIndex).name, score*10);
	}
